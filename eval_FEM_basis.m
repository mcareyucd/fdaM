...
J = zeros(size(t,1),1);
for k=1:size(t,1)
J(k)  = det([1 v1(k,1) v1(k,2); 1 v2(k,1) v2(k,2); 1 v3(k,1) v3(k,2)]);
end
    ...
    if ~isnan(indi)
        ...
        if order == 1
            ...
            elseif nderivs(1) == 1 && nderivs(2) == 0
                ...
                evalmat(i,nodeindex(indi,2)) = M2(indi(2),2);
                evalmat(i,nodeindex(indi,3)) = M3(indi(3),2);
            elseif nderivs(1) == 0 && nderivs(2) == 1
                evalmat(i,nodeindex(indi,1)) = M1(indi(1),3);
                evalmat(i,nodeindex(indi,2)) = M2(indi(2),3);
                evalmat(i,nodeindex(indi,3)) = M3(indi(3),3);
            else
